...
object TweetLength {
  ...
  def colorForRemainingCharsCount(remainingCharsCount: Signal[Int]): Signal[String] = {
    Signal(calculateColor(remainingCharsCount()))
  }

  private def calculateColor(remaining: Int): String = remaining match {
    case it if (0 to 14) contains it  => "orange"
    case it if it >= 15  => "green"
    case _ => "red"
  }
